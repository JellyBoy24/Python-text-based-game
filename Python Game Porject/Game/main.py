from Game.entities import Entity, Dummy
...
def main():
    ...
    while not tut_won:
        tut_won = tutorial_fight_sequence(player, Dummy)
        if tut_won:
    ...
    while True:
        ...
        if action == 1:
            ...
            if local_map[y_pos][x_pos] == "Castle interior":
                if not interacted:
        ...
        if moved:
            ...
            if player.position == local_map[3][3]: # checks if on the training arena
                ...
                if input("go inside?(y/n): ").lower() == "y":
                    fight_win = training_fight_sequence(player, Dummy)
                    if fight_win:
